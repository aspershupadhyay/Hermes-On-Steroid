...
export interface DesignStudioProps {
  ...
  /** Called when batch pre-render finishes — Studio is ready for image gen */
  onBatchRenderComplete?: (posts: Post[]) => void
  isActive?: boolean
...
export default function DesignStudio({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      // Tell App we're done — Studio is ready for image gen, no more arbitrary sleeps
      onBatchRenderComplete?.(posts)
    }, 200)
    return (): void => { clearTimeout(timer) }
  }, [pendingBatch])
  ...
  // Stable version counter — increments whenever pages state changes so the
  // images_ready watcher re-fires ONLY when pages actually changed, not on
  // every render that produces a new pages array reference.
  const [pagesVersion, setPagesVersion] = useState(0)
  useEffect(() => { setPagesVersion(v => v + 1) }, [pages])
