      ...
      {expanded && (
        <div style={{ padding: '12px 14px', background: T.bg3, display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <label
              title="AI generates 5 candidates, scores each on hook strength, specificity, curiosity gap, trending signal and shareability — outputs the highest scorer"
              style={{
                display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, cursor: 'pointer',
                color: field.ranked ? T.accent : T.text2,
                padding: '3px 10px', borderRadius: 6,
                border: `1px solid ${field.ranked ? T.accent + '60' : T.border}`,
                background: field.ranked ? T.accent + '18' : 'transparent',
                transition: 'all .15s',
              }}>
              <input type="checkbox" checked={!!field.ranked}
                onChange={e => onChange({ ...field, ranked: e.target.checked })}
                style={{ display: 'none' }} />
              {field.ranked ? '★' : '☆'} Rank
            </label>
          </div>
