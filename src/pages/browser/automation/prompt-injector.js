...
exports.dismissComparisonModal = dismissComparisonModal;
exports.waitForResponseComplete = waitForResponseComplete;
const browser_controller_1 = require("./browser-controller");
const INJECT_TIMEOUT_MS = 60000;
/**
...
async function injectPrompt(prompt) {
  ...
  `;
    return new Promise((resolve) => {
        const timer = setTimeout(() => resolve({ success: false, method: 'failed', error: 'Inject timeout' }), INJECT_TIMEOUT_MS);
        (0, browser_controller_1.execInWindow)(script)
            .then((r) => {
            clearTimeout(timer);
            resolve(r.ok
                ? { success: true, method: r.method }
 ...
 */
async function dismissComparisonModal() {
    const script = `
    (() => {
  ...
  `;
    try {
        const dismissed = await (0, browser_controller_1.execInWindow)(script);
        if (dismissed) {
 ...
 */
function waitForResponseComplete(timeoutMs = 180000) {
    const script = `
    new Promise((resolve) => {
  ...
  `;
    return (0, browser_controller_1.execInWindow)(script).catch(() => false);
}
