...
// ── Handle renderer constants (module scope so ungroupSelected can call them) ──
const H_ACCENT   = '#C96A42'
const H_CORNER_R = 11
const H_PILL_L   = 16
const H_PILL_S   = 9
const H_SW       = 1.5

const _renderCorner = (ctx: CanvasRenderingContext2D, left: number, top: number): void => {
  ctx.save()
  ctx.shadowColor = 'rgba(0,0,0,0.30)'; ctx.shadowBlur = 6; ctx.shadowOffsetY = 2
  ctx.beginPath(); ctx.arc(left, top, H_CORNER_R, 0, Math.PI * 2)
  ctx.fillStyle = '#FFFFFF'; ctx.fill()
  ctx.shadowColor = 'transparent'; ctx.strokeStyle = H_ACCENT; ctx.lineWidth = H_SW; ctx.stroke()
  ctx.restore()
}
const _renderEdge = (ctx: CanvasRenderingContext2D, left: number, top: number, _s: unknown, _o: unknown, key: string): void => {
  const isH = key === 'mt' || key === 'mb'
  const hw = isH ? H_PILL_L : H_PILL_S; const hh = isH ? H_PILL_S : H_PILL_L
  ctx.save()
  ctx.shadowColor = 'rgba(0,0,0,0.22)'; ctx.shadowBlur = 5; ctx.shadowOffsetY = 2
  ctx.beginPath(); ctx.roundRect(left - hw, top - hh, hw * 2, hh * 2, Math.min(hw, hh))
  ctx.fillStyle = '#FFFFFF'; ctx.fill()
  ctx.shadowColor = 'transparent'; ctx.strokeStyle = H_ACCENT; ctx.lineWidth = H_SW; ctx.stroke()
  ctx.restore()
}
const applyHandleStyle = (controls: Record<string, fabric.Control>): void => {
  ;(['tl', 'tr', 'bl', 'br'] as const).forEach(k => {
    if (!controls[k]) return
    controls[k].render = _renderCorner
    controls[k].sizeX = (H_CORNER_R + 6) * 2; controls[k].sizeY = (H_CORNER_R + 6) * 2
  })
  ;(['mt', 'mb', 'ml', 'mr'] as const).forEach(k => {
    if (!controls[k]) return
    controls[k].render = (ctx, l, t, s, o) => _renderEdge(ctx, l, t, s, o, k)
    controls[k].sizeX = (k === 'ml' || k === 'mr') ? (H_PILL_S + 6) * 2 : (H_PILL_L + 6) * 2
    controls[k].sizeY = (k === 'mt' || k === 'mb') ? (H_PILL_S + 6) * 2 : (H_PILL_L + 6) * 2
  })
...
const DesignCanvas = forwardRef<CanvasHandle, CanvasProps>((
  ...
  // Group editing state (Figma-style double-click to enter group)
  const groupEditRef      = useRef<{ children: FabricObject[]; label: string; opacity: number; groupId: string } | null>(null)
  const exitGroupEditRef  = useRef<((regroup?: boolean) => void) | null>(null)
  const enterGroupEditRef = useRef<((group: fabric.Group, pointer: { x: number; y: number }) => void) | null>(null)
  ...
  const ungroupSelected = useCallback((): void => {
    const c = fabricRef.current; if (!c) return
    if (groupEditRef.current) exitGroupEditRef.current?.(false)
    const active = c.getActiveObject()
    // ActiveSelection extends Group, so check it FIRST to avoid false-positive on instanceof Group.
    // Also guard against non-Group objects (plain elements).
    if (active instanceof fabric.ActiveSelection || !(active instanceof fabric.Group)) return

    const group        = active as fabric.Group
    const children     = group.getObjects() as FabricObject[]
    const groupMatrix  = group.calcTransformMatrix()   // snapshot while group is on canvas
    const groupOpacity = group.opacity ?? 1

    // Snapshot world transforms BEFORE removing the group.
    // Use calcOwnMatrix() (local-only) so the group matrix isn't applied twice.
    // translateX/Y from qrDecompose is the center — use setPositionByOrigin.
    const worldData = children.map(child => ({
      obj:         child,
      worldMatrix: fabric.util.multiplyTransformMatrices(groupMatrix, child.calcOwnMatrix()),
      localOpacity: (child.opacity ?? 1) * groupOpacity,
    }))

    c.remove(active as unknown as FabricObject)
    c.discardActiveObject()

    const added: FabricObject[] = []
    worldData.forEach(({ obj, worldMatrix, localOpacity }) => {
      ;(obj as FabricObject & { group?: unknown }).group = undefined
      const { angle, scaleX, scaleY, skewX, skewY, translateX, translateY } =
        fabric.util.qrDecompose(worldMatrix)
      obj.set({ angle, scaleX, scaleY, skewX: skewX || 0, skewY: skewY || 0, flipX: false, flipY: false, opacity: localOpacity })
      obj.setPositionByOrigin(new fabric.Point(translateX, translateY), 'center', 'center')
      c.add(obj)       // add first so canvas registers absolute coords
      obj.setCoords()  // then refresh hit-test cache
      added.push(obj)
    })

    if (added.length > 1) {
      // Build ActiveSelection and apply new-style handles to it explicitly,
      // since object:added doesn't fire for the ActiveSelection itself.
      const sel = new fabric.ActiveSelection(added, { canvas: c })
      applyHandleStyle(sel.controls as Record<string, fabric.Control>)
  ...
  // ── Group editing (Figma/Canva double-click to enter group) ──────────────

  const exitGroupEdit = useCallback((regroup = true): void => {
    const c = fabricRef.current; if (!c) return
    const state = groupEditRef.current
    if (!state) return
    groupEditRef.current = null

    if (regroup) {
      const { groupId, label, opacity } = state
      const children = c.getObjects().filter(
        o => (o as FabricObject & { _eliteGroupId?: string })._eliteGroupId === groupId
      ) as FabricObject[]
      if (children.length > 0) {
        c.discardActiveObject()
        children.forEach(o => {
          ;(o as FabricObject & { _eliteGroupId?: string })._eliteGroupId = undefined
          c.remove(o)
        })
        const newGroup = new fabric.Group(children as FabricObject[])
        newGroup.eliteType = 'group'
        newGroup.eliteLabel = label
        newGroup.opacity = opacity
        c.add(newGroup as unknown as FabricObject)
        c.setActiveObject(newGroup as unknown as FabricObject)
      }
    } else {
      // Exit group edit WITHOUT regrouping — ungroup children to canvas level.
      // Re-add children so they become regular canvas objects, then clean up.
      state.children.forEach(o => {
        ;(o as FabricObject & { _eliteGroupId?: string })._eliteGroupId = undefined
        c.add(o)
      })
      c.discardActiveObject()
  ...
  const enterGroupEdit = useCallback((group: fabric.Group, pointer: { x: number; y: number }): void => {
    const c = fabricRef.current; if (!c) return
    if (groupEditRef.current) exitGroupEdit()

    const label       = (group as unknown as FabricObject).eliteLabel || 'Group'
    const opacity     = group.opacity ?? 1
    const groupId     = `ge_${Date.now()}`
    const children    = group.getObjects() as FabricObject[]
    ...
    c.remove(group as unknown as FabricObject)
    c.discardActiveObject()

    const placed: FabricObject[] = []
    worldData.forEach(({ obj, worldMatrix }) => {
      ;(obj as FabricObject & { group?: unknown }).group = undefined
      const { angle, scaleX, scaleY, skewX, skewY, translateX, translateY } =
        fabric.util.qrDecompose(worldMatrix)
      obj.set({ angle, scaleX, scaleY, skewX: skewX || 0, skewY: skewY || 0, flipX: false, flipY: false })
      obj.setPositionByOrigin(new fabric.Point(translateX, translateY), 'center', 'center')
      ;(obj as FabricObject & { _eliteGroupId?: string })._eliteGroupId = groupId
      obj.setCoords()
      c.add(obj)
      placed.push(obj)
    })

    groupEditRef.current = { children: placed, label, opacity, groupId }

    const clicked = [...placed].reverse().find(item =>
      item.containsPoint(new fabric.Point(pointer.x, pointer.y))
    )
    if (clicked) c.setActiveObject(clicked)
    else if (placed.length > 0) c.setActiveObject(placed[placed.length - 1])
    c.requestRenderAll()
  }, [exitGroupEdit])
  enterGroupEditRef.current = enterGroupEdit
  ...
  useEffect(() => {
    ...
    // ── Swift-style selection handles (use module-level applyHandleStyle) ──────

    // Patch controls on an object; recurse into regular Group children.
    // Also patches ActiveSelection so multi-select outer box uses the new handles.
    const patchObjControls = (obj: fabric.FabricObject): void => {
      const controls = obj.controls as Record<string, fabric.Control>
      if (!controls) return
      applyHandleStyle(controls)
      if (obj instanceof fabric.Group && !(obj instanceof fabric.ActiveSelection)) {
        ;(obj as fabric.Group).getObjects().forEach(child =>
          patchObjControls(child as fabric.FabricObject)
        )
      }
    }
    ...
    Object.assign(fabric.FabricObject.ownDefaults, {
      ...
      cornerColor:        '#FFFFFF',
      cornerStrokeColor:  H_ACCENT,
      cornerSize:         H_CORNER_R * 2,
      cornerStyle:        'rect',
      borderColor:        H_ACCENT,
      borderScaleFactor:  6,
      padding:            0,
      snapAngle:          15,
      hasBorders:              true,
    ...
    if (fabric.IText?.ownDefaults) {
      (fabric.IText.ownDefaults as Record<string, unknown>).editingBorderColor = H_ACCENT
    }
    ...
    // ── Figma/Canva-style click-through for ActiveSelection ──────────────────
    // Fabric v6 findTarget() returns the ActiveSelection itself when you click
    // within its bounds — individual members are never the target.  We fix this
    // by recording the click point on mouse:down and, if no drag occurred,
    // testing each member with containsPoint() on mouse:up.
    let _selClickPt: fabric.Point | null = null

    canvas.on('mouse:up', (opt) => {
      onGuidesChange?.(null)

      const ptr = _selClickPt
      _selClickPt = null
      if (!ptr) return

      // Compare down vs up positions — skip if the user dragged
      const upEvt = (opt as unknown as { e?: MouseEvent }).e
      if (upEvt) {
        const upPt = canvas.getScenePoint(upEvt)
        if (Math.abs(upPt.x - ptr.x) > 4 || Math.abs(upPt.y - ptr.y) > 4) return
      }

      const active = canvas.getActiveObject()
      if (!(active instanceof fabric.ActiveSelection)) return

      const members = active.getObjects() as FabricObject[]
      // Reverse so topmost object (highest z) wins
      const hit = [...members].reverse().find(m => m.containsPoint(ptr))
      if (hit) {
    ...
    canvas.on('mouse:down', (opt) => {
      ...
      // Record pointer for click-through logic above
      _selClickPt = null
      if (e?.button === 0) {
        const active = canvas.getActiveObject()
        if (active instanceof fabric.ActiveSelection && active.getObjects().length > 1) {
          _selClickPt = canvas.getScenePoint(e)
        }
      }
    })

    canvas.on('mouse:dblclick', (rawOpt) => {
      const opt = rawOpt as unknown as { target?: FabricObject; e?: MouseEvent }
      const target = opt.target; if (!target) return
      // Enter group editing mode (Figma-style isolation)
      if (target instanceof fabric.Group && !(target instanceof fabric.ActiveSelection)) {
        const scenePoint = canvas.getScenePoint(opt.e!)
        enterGroupEditRef.current?.(target as fabric.Group, scenePoint)
        return
      }
  ...
  useEffect(() => {
    const down = async (e: KeyboardEvent): Promise<void> => {
      ...
      // ── Escape → exit group edit, then deselect ─────────────────────────
      if (e.key === 'Escape') {
        if (groupEditRef.current) { exitGroupEditRef.current?.(true); return }
    ...
    return (): void => { window.removeEventListener('keydown', down); window.removeEventListener('keyup', up) }
      ...
      ungroupSelected, exitGroupEdit, bringToFront, sendToBack, bringForward, sendBackward,
