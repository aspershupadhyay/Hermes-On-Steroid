...
function applyColorMultiply(c: HTMLCanvasElement, r: number, g: number, b: number): void {
...
function genNoise(lo = 200, hi = 255): HTMLCanvasElement {
...
function genPaper(rough: boolean): HTMLCanvasElement {
  const c = document.createElement('canvas'); c.width = c.height = 256
  const ctx = c.getContext('2d')!
  const id = ctx.createImageData(256, 256); const d = id.data
...
function genVintage(): HTMLCanvasElement {
  const c = document.createElement('canvas'); c.width = c.height = 256
  const ctx = c.getContext('2d')!
  const id = ctx.createImageData(256, 256); const d = id.data
  for (let i = 0; i < d.length; i += 4) {
...
function genGrunge(dark: boolean): HTMLCanvasElement {
  const c = document.createElement('canvas'); c.width = c.height = 256
  const ctx = c.getContext('2d')!
  const id = ctx.createImageData(256, 256); const d = id.data
  for (let i = 0; i < d.length; i += 4) {
...
function genChroma(): HTMLCanvasElement {
  const c = document.createElement('canvas'); c.width = c.height = 256
  const ctx = c.getContext('2d')!
  const id = ctx.createImageData(256, 256); const d = id.data
  for (let i = 0; i < d.length; i += 4) {
...
function genHalftone(spacing: number): HTMLCanvasElement {
  const c = document.createElement('canvas'); c.width = c.height = 256
  const ctx = c.getContext('2d')!
...
function genLines(gap: number): HTMLCanvasElement {
  const c = document.createElement('canvas'); c.width = c.height = 256
  const ctx = c.getContext('2d')!
  ctx.fillStyle = '#ffffff'; ctx.fillRect(0, 0, 256, 256)
...
function genLinen(): HTMLCanvasElement {
 ...
 * Pass hexColor (e.g. '#c05030') to colorize the texture via multiply blend.
 * White (#ffffff) or omitted = original appearance.
 */
export function getPresetSrc(id: string, hexColor?: string): string {
  const isColored = hexColor && hexColor.toLowerCase() !== '#ffffff'
  const cacheKey  = isColored ? `${id}_${hexColor!.toLowerCase()}` : id
  if (srcCache.has(cacheKey)) return srcCache.get(cacheKey)!
  const gen = GENERATORS[id]
  if (!gen) return ''
  const canvas = gen()
  if (isColored) {
    const [r, g, b] = hexToRgb(hexColor!)
    applyColorMultiply(canvas, r, g, b)
  }
