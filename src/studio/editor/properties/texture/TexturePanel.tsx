...
export function TexturePanel({ params, onChange, onClear, selectionCharCount = 0 }: TexturePanelProps): JSX.Element {
  ...
  const selectPreset = (id: string, _src: string): void => {
    const coloredSrc = getPresetSrc(id, presetColor)
    const merged     = presetToParams(id, coloredSrc)
    onChange({ ...DEFAULT_TEXTURE, ...merged })
  }

  const handlePresetColorChange = (hex: string): void => {
    setPresetColor(hex)
    if (!params.presetId) return
    const coloredSrc = getPresetSrc(params.presetId, hex)
    onChange({ ...params, src: coloredSrc })
  }
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        ...
        {/* ── Preset color picker (shown only when a preset is active) ── */}
        {params.presetId && (
          <div className="flex items-center justify-between px-2 py-1.5 rounded-lg bg-white/[0.04] border border-white/[0.07]">
            <span className="text-[10px] text-warm-faint">Texture Color</span>
            <div className="flex items-center gap-2">
              <ColorPicker value={presetColor} onChange={handlePresetColorChange} />
              {presetColor.toLowerCase() !== '#ffffff' && (
                <button
                  onClick={() => handlePresetColorChange('#ffffff')}
                  className="text-[9px] text-warm-faint hover:text-accent cursor-pointer transition-colors">
                  Reset
                </button>
              )}
            </div>
      ...
      <div className="space-y-1.5">
        ...
        <ColorPicker value={params.tintColor} onChange={v => up('tintColor', v)} />
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-warm-faint w-[52px] shrink-0">Strength</span>
