...
def build_xml_system_prompt(
    ...
    # Title length — hard minimum, never short or truncated
    parts.append(
        f"\n## TITLE LENGTH\n"
        f"Every title MUST be at least {title_min_length} characters and no more than {title_max_length} characters.\n"
        f"NEVER output a title shorter than {title_min_length} characters — if your draft is too short, expand it before writing it.\n"
        f"The title must be COMPLETE — do not cut off mid-sentence or mid-phrase.\n"
        f"Count characters before finalizing. Enforce strictly."
    )
    ...
    # Ranked-field instruction — injected once when any enabled field has ranked=True
    enabled = [f for f in output_fields if f.get("enabled", True)]
    ranked_fields = [f.get("id") or f.get("key", "") for f in enabled if f.get("ranked")]
    if ranked_fields:
        flist = ", ".join(f"<{fid}>" for fid in ranked_fields)
        parts.append(
            f"\n## RANKED GENERATION\n"
            f"For the following field(s): {flist}\n"
            f"Before writing the final value, silently generate 5 distinct candidates.\n"
            f"Score each candidate on these 5 axes (0-10 each):\n"
            f"  1. Hook strength — does it stop a scroll?\n"
            f"  2. Specificity — named actors, real numbers, concrete facts\n"
            f"  3. Curiosity gap — does it make the reader need to know more?\n"
            f"  4. Trend alignment — is it connected to what people are talking about right now?\n"
            f"  5. Shareability — would someone screenshot or repost this?\n"
            f"Sum the scores. Output ONLY the highest-scoring candidate inside the XML tag.\n"
            f"Do NOT mention the candidates, scores, or ranking process in your output."
        )
...
class NvidiaRAG:
    ...
    def generate_content(
        ...
        citation_rule = (
            ...
            "Do NOT embed raw source citations, URLs, or [SOURCE: ...] blocks inside any output field — "
            "write naturally, like a human. Sources are listed separately in the SOURCES block only."
    ...
    def generate_instagram(self, topic: str, include_9x16=False, include_hook=False,
        ...
        default_fields = [
            ...
            {"id": "caption",         "label": "Caption",         "aiHint": "800-1200 character social media caption. Write clearly, no jargon, no raw URLs or source citations inside the caption.", "type": "text", "enabled": True},
    ...
    async def stream_batch_generate(
        ...
        citation_rule = (
            "\n\nSOURCE RULE: Every fact MUST come from the research context. No fabrication. "
            "Do NOT embed raw source citations, URLs, or [SOURCE: ...] blocks inside any output field — "
            "write naturally, like a human. Sources are listed separately in the SOURCES block only."
        )
